...
class ConvertPanel(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        files_label = tk.Label(
            self,
            text="pkl and npy files of summary/raw data for a run:",
            font="TkDefaultFont 13 bold",
        )
        self.files = MenuOptions(self)
        name_label = tk.Label(
            self,
            text="hdf5 file name: (will append .hdf5 to what is given)",
            font="TkDefaultFont 13 bold",
        )
    ...
    def export_data(self):
        if os.path.exists(self.files.summary_directory) and os.path.exists(
            self.files.raw_directory
        ):
            summary_data = np.load(
                self.files.summary_directory, allow_pickle=True, encoding="latin1"
            )
