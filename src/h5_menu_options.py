...
class H5MenuOptions(tk.Frame):
    ...
    def choose_h5_data(self):
        ...
        directory = (
            self.h5_directory[(self.h5_directory.rfind("/") + 1) :]
            if len(self.h5_directory) > 25
            else self.h5_directory
        )
        self.h5_text.set(directory)

    def delete(self):
        for widget in [self.h5_label, self.h5_directory, self.h5_chooser]:
            widget.grid_forget()
            widget.destroy()
