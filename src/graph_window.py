...
class GraphWindow(tk.Toplevel):
    def __init__(self, parent, df_runs, raw_data, *args, **kwargs):
        ...
        group_select = tk.OptionMenu(
            control_frame, var_group, *group_options, command=self.update_group_category
        )
        undo_button = tk.Button(
            control_frame, text="Undo Last Deletion", width=15, command=self.undo_delete
        )
        show_histogram_button = tk.Button(
            control_frame, text="Show Histogram", width=15, command=self.show_histogram
        )
        ...
        delete_button = tk.Button(
            delete_frame, text="Delete Groups", width=15, command=self.delete_group
        )
        ...
        clear_selected_button = tk.Button(
            selection_frame, text="Unselect all", width=15, command=self.clear_selected
        )
        show_raw_data_button = tk.Button(
            selection_frame, text="Show Raw Data", width=15, command=self.showRawData
        )
        self.is_box = tk.IntVar()
        box_select_on = tk.Checkbutton(
            selection_frame, text="Box Select", variable=self.is_box, command=self.activate_box
        )
        delete_unselected_button = tk.Button(
            selection_frame, text="Delete Unselected", width=15, command=self.delete_unselected
        )
        ...
        export_data_button = tk.Button(
            export_frame, text="Export Data", width=15, command=self.export_data
        )
        ...
        self.selected, = self.ax.plot(xs, ys, "o", ms=10, alpha=0.8, color="red", visible=False)
        self.lines = []
        self.data = None
        self.regroup_data()
        ...
        self.rs = RectangleSelector(
            self.ax, self.line_select_callback, drawtype="box", useblit=True, interactive=True
        )
    ...
    def regroup_data(self):
        ...
        for label, group in self.data:
            line, = self.ax.plot(
                group[self.xaxis], group[self.yaxis], "o", ms=3, alpha=0.3, picker=2, label=label
            )
    ...
    def line_select_callback(self, eclick, erelease):
        if self.is_box.get():
            ...
            self.selected_points.update(
                self.runs[
                    (self.runs[self.xaxis] >= x_1)
                    & (self.runs[self.xaxis] <= x_2)
                    & (self.runs[self.yaxis] >= y_1)
                    & (self.runs[self.yaxis] <= y_2)
                ].index
            )
            self.update()
    ...
    def delete_unselected(self):
        oldRuns = self.runs
        toKeep = [
            int(self.runs.iloc[i].name) in self.selected_points
            for i in range(0, len(self.runs.index))
        ]
