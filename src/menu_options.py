...
class MenuOptions(tk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.summary_label = tk.Label(
            self, text="Summary data file:", font="TkDefaultFont 13 bold"
        )
        self.summary_text = tk.StringVar()
        self.summary_directory = tk.Label(self, textvariable=self.summary_text)
        self.summary_chooser = tk.Button(
            self, text="Choose Summary Data", command=self.choose_summary_data
        )
    ...
    def choose_summary_data(self):
        ...
        directory = (
            self.summary_directory[(self.summary_directory.rfind("/") + 1) :]
            if len(self.summary_directory) > 25
            else self.summary_directory
        )
    ...
    def choose_raw_data(self):
        ...
        directory = (
            self.raw_directory[(self.raw_directory.rfind("/") + 1) :]
            if len(self.raw_directory) > 25
            else self.raw_directory
        )
