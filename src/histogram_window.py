...
class HistogramWindow(tk.Toplevel):
    def __init__(self, parent, df_runs, *args, **kwargs):
        ...
        histogram_select = tk.OptionMenu(
            control_frame, hist_var, *options, command=self.change_histogram
        )
        undo_delete_button = tk.Button(
            control_frame, text="Undo last deletion", command=self.undo_delete
        )
        ...
        delete_channel_cutoff = tk.Button(
            channel_delete_frame, text="Delete Above Cutoff", command=self.delete_above_cutoff
        )
        ...
        delete_range = tk.Button(
            range_delete_frame, text="Delete Specified Range", command=self.delete_in_range
        )
                ...
                for run, group in self.data.groupby("run"):
                    for channel, subgroup in group.groupby("channel"):
                        if len(subgroup.index) >= cutoff:
                            to_keep = to_keep[
                                (to_keep["run"] != run) | (to_keep["channel"] != channel)
                            ]
    ...
    def plot_by_run(self):
        ...
        plot_groups = [
            run_groups.get_group(key)[self.x_statistic] for key in run_groups.groups.keys()
        ]
        # plot_groups.append([])
